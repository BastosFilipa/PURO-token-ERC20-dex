...
contract("Exchange", ([deployer, feeAccount, user1, user2]) => {
  ...
  describe("depositing Ether", async () => {
    ...
    it("tracks the Ether deposit", async () => {
      ...
      console.log("Balance:", balance.toString()); // Add this line to inspect the balance
      balance.toString().should.equal(amount.toString());
    });

    it("emits a Deposit event", async () => {
      const log = result.logs.find((log) => log.event === "Deposit");
      assert.isDefined(log, "Deposit event was not emitted");

      console.log("Event Log:", log); // Inspect this line
    ...
    describe("filling orders", async () => {
      ...
      describe("success", async () => {
        ...
        it("executes the trade & charges fees", async () => {
          ...
          balance
            .toString()
            .should.equal(ether(1).toString(), "user2 receive ether");
          ...
          balance
            ...
            .should.equal(
              tokens(0.9).toString(),
              "user2 tokens dedutected with fee applied"
            );
        ...
        it("emits a 'Trade' event", async () => {
          ...
          event.amountGet
            .toString()
            .should.equal(tokens(1).toString(), "amount is correct");
          event.tokenGive.should.equal(ETHER_ADDRESS, "tokenGive is correct");
          event.amountGive
            .toString()
            .should.equal(ether(1).toString(), "amountGive is correct");
